...
async function checkSitesFeeds() {
  ...
  // 오늘식당 조회 및 슬랙 메시지 전송
  console.log(":: Start Check Oneul Feeds");
  ...
  console.log(":: End Check Oneul Feeds(" + oneulMenu.length + ")");

  if (oneulMenu.length > 0) {
    console.log(":: Send Slack Message - Oneul Feeds");
  ...
  // 5분 후 재조회
  setTimeout(checkSitesFeeds, 5 * 60 * 1000);
  console.groupEnd("");
  console.log("");
}
